...
async function _endTurnCore(){
  // [slice 1·2026-05-07] 新管道 feature flag·observer 模式·不影响旧路径
  // 启用：控制台 P.flags = {useNewPipeline:true}·下回合可见 [pipeline] 日志
  // slice 2 起 prep step 真接力·legacy 检查接力 flag 跳过双执行
  // [slice 5·2026-05-07] pipeline.run 移到 'await EndTurnHooks.execute(before)' 之后
  ...
  // [slice 5·2026-05-07] pipeline.run·必须在 before-hooks 之后·让 systems 等步骤看到 hook mutation
  try {
    if (typeof P !== 'undefined' && P.flags && P.flags.useNewPipeline
        && window.TM && TM.Endturn && TM.Endturn.Pipeline) {
      _obsCtx = TM.Endturn.Pipeline.buildCtx();
      await TM.Endturn.Pipeline.run(_obsCtx);
    }
  } catch(_pipeObsE) {
    try { console.warn('[pipeline] observer run failed (legacy unaffected)', _pipeObsE); } catch(_){}
    _obsCtx = null; // 失败时 fallback 到 legacy
  }

  // [slice 2.5·2026-05-07] _prepDone Set·pipeline 已完成的 phase·legacy 按集合跳过避免双执行
  // pipeline 失败 fallback：_obsCtx=null 时 set 空·legacy 全跑
  // pipeline 部分失败：set 仅含成功 phase·legacy 补跑剩余的
  var _prepDone = (_obsCtx && _obsCtx.input && Array.isArray(_obsCtx.input._completedPrepPhases))
    ? new Set(_obsCtx.input._completedPrepPhases)
    : new Set();

  // Phase 0-A·情节弧兜底·若 >=4 回合未更新则触发后台推进(不等待·不阻塞)
  if (!_prepDone.has('0-A')) {
    try {
  ...
  // Phase 0-1: 初始化 + 收集输入·slice 2 起接 pipeline 输出避免双执行
  var npcContext, input;
  if (_prepDone.has('0-1')) {
    npcContext = _obsCtx.input.npcContext;
    input = _obsCtx.input;
  } else {
    npcContext = _endTurn_init();
    input = _endTurn_collectInput();
  }
  ...
  // Phase 1.75·NPC AI 决策器 + Phase 1.8·长期行动摘要·slice 2.6 起接 pipeline kickoff 避免 token 双烧
  var _preThreeSystemsP = null;
  var _preLongTermP = null;
  if (_obsCtx && _obsCtx.input && _obsCtx.input._planPrefetchKickedOff) {
    _preThreeSystemsP = (_obsCtx.subcalls && _obsCtx.subcalls.preThreeSystemsP) || null;
    _preLongTermP = (_obsCtx.subcalls && _obsCtx.subcalls.preLongTermP) || null;
  } else {
    // legacy kickoff·pipeline off / plan-prefetch 步骤未跑时使用
    try {
  ...
  // slice 3a·2026-05-07·接 pipeline ai step 输出避免双 token 烧
  var aiResult;
  if (_obsCtx && _obsCtx.input && _obsCtx.input._aiInferRan && _obsCtx.results && _obsCtx.results.aiResult) {
    aiResult = _obsCtx.results.aiResult;
  } else {
    aiResult = await _endTurn_aiInfer(edicts, xinglu, memRes, oldVars);
  }
  ...
  // Phase 2.5/2.6·[slice 4·2026-05-07] pipeline post-ai-edict 跑过则全跳避免双重 mutation
  var _postAiEdictDone = !!(_obsCtx && _obsCtx.input && _obsCtx.input._postAiEdictRan);
  ...
  // Phase 2.6: 应用昏君活动效果·pipeline 已计算则取 ctx.results.tyrantResult
  var tyrantResult = null;
  if (_postAiEdictDone) {
    tyrantResult = (_obsCtx.results && _obsCtx.results.tyrantResult) || null;
  } else {
  ...
  // Phase 3 + 3.5·[slice 5·2026-05-07] pipeline systems 跑过则全跳避免双 GM.turn++ 灾难
  var _systemsDone = !!(_obsCtx && _obsCtx.input && _obsCtx.input._systemsRan);
  // Phase 3: 系统更新
  var queueResult;
  if (_systemsDone) {
    queueResult = (_obsCtx.results && _obsCtx.results.queueResult) || null;
  } else {
    queueResult = await _endTurn_updateSystems(timeRatio, zhengwen);
  }
  ...
  // Phase 4-5·[slice 6·2026-05-07] pipeline render-finalize 跑过则全跳·deferred 路径仍由 legacy 处理
  var _renderFinalizeDone = !!(_obsCtx && _obsCtx.input && _obsCtx.input._renderFinalizeRan);
  ...
  // 注：deferred 路径 (shijiModal courtDone === false) pipeline 没接·此处 legacy 仍跑
  if (!_renderFinalizeDone) {
    if (GM._pendingShijiModal && GM._pendingShijiModal.courtDone === false) {
