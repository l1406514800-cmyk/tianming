...
async function _endTurnCore(){
  ...
  // [slice 7a·2026-05-07] 默认翻转·P.flags.useNewPipeline === false 才回退 legacy
  // 启用：默认即开·关闭：P.flags = {useNewPipeline:false}
  ...
  // [slice 7a·2026-05-07] 翻转默认·flag 不显式 false 即视为开·pipeline 成生产路径
  try {
    var _pipelineFlag = !(typeof P !== 'undefined' && P.flags && P.flags.useNewPipeline === false);
    if (_pipelineFlag && window.TM && TM.Endturn && TM.Endturn.Pipeline) {
