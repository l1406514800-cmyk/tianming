...
### `ref-check.js`（R136 新增）

扫描所有 `*.html` / `*.js` 里的 `<script src="..."/>` 和 `fetch('xxx')` 本地引用，验证目标文件存在。

```bash
node scripts/ref-check.js
```

**能防住**：拆分/改名后忘记更新某个 HTML 的 script 引用（R136 就是此类：R122 拆了 tm-economy-military.js 但漏改 editor.html）。

这类 bug 在 `syntax-check` 和 `headless-smoke` 里都无法检出（前者只看单文件语法，后者只加载 index.html），所以需要专门的跨文件引用检查。
...
node scripts/ref-check.js            # 所有 script/fetch 引用必须存在
```

如 pass 数下降或出现新 fail，说明拆分引入回归。回滚或修正。
如 ref-check 报断链，说明某个 HTML 还在引用被拆/被删的老文件。
